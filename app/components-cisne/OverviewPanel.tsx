  ...
  return (
    <section className="rounded-[2rem] bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.08)] lg:p-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm shadow-sm">
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_240px]">
        <div>
          <div className="flex h-64 items-end gap-3 rounded-[1.75rem] bg-slate-50 px-4 pb-4 pt-8">
            {overview.topCharges.map((charge) => (
              <div key={charge.label} className="flex flex-1 flex-col items-center gap-3">
                ...
                <span className="text-center text-xs font-semibold leading-4 text-slate-500">
                  {charge.label}
                </span>
              </div>
            ))}
            ...
            {overview.history.map((item) => (
              <div key={item.label}>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-slate-900">{item.amountLabel}</span>
                </div>
